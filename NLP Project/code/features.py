...
def extract_feature(preprocess, preprocessed_train_tweets,preprocessed_test_tweets,feature_no=0,train_save_path='',test_save_path=''):
    if feature_no==0:
        ...
        def labelize_tweets_ug(tweets,label):
            ...
            for i, t in enumerate(tweets):
                result.append(TaggedDocument(str(t).split(), [prefix + '_%s' % i]))
            return result
        ...
        def twitter_embedding(feature_no,tweets_text,word_size,CBOW=True):
            ...
            model_ug_cbow.save('..//features//w2v_model_ug_cbow (feature_'+str(feature_no)+'.word2vec')
        ...
        if ((feature_no>=14) & (feature_no<=19)):
            ...
            try:
                ...
                additionalTweets = pd.read_csv(open('..//preprocessed_data//additional_Tweets.csv','rU',encoding='utf-8'),encoding='utf-8',engine='c')
        ...
        for no,doc in enumerate(preprocessed_train_tweets):
            weighted_glove=np.zeros(word_size)
            doc_length=0
            for i in doc.split():
                doc_length+=1
        ...
        def Glove_embedding(tweets,tfidf_words_list,tfidf_documents):
            print('read GloV...')
            gloV={}
            with open("G://NLP project//data//glove.6B//glove.6B.300d.txt",'rb') as f:  
                line = f.readline()
                no = 1
                while line:
                    print(no)
    ...
    elif (feature_no==7)|(feature_no==20)|(feature_no==21):    
        # Sentiment Feature
        train_tagged_texts = [nltk.pos_tag(word_tokenize(t)) for t in preprocessed_train_tweets]
        test_tagged_texts = [nltk.pos_tag(word_tokenize(t)) for t in preprocessed_test_tweets]
        ...
        for index,tagged in enumerate(train_tagged_texts):
            ...
            for (word, tag) in tagged:
                ...
                if ss_set:
                    ...
                    word_sentiment=ss_set.pos_score()-ss_set.neg_score()
                    if abs(word_sentiment)>abs(maximum):
                        maximum=word_sentiment
                    if (ss_set.obj_score()>=ss_set.pos_score())&(ss_set.obj_score()>=ss_set.neg_score()):
                        o_count+=1
            ...
            if feature_no==7:
                ...
                append=[p_count,n_count, o_count]
            X.append(append)
        ...
        for n in [1,2,3]:
            if n==1:
                ...
                for skip in [1,2]:
                    ...
                    for tweet in preprocessed_train_tweets:
                        tweet=tweet.split()
                        all_skip_ngrams.extend([' '.join(tweet[index:index+(n-1)*(skip+1)+1:(skip+1)]) for index in range(len(tweet)) if (index+(n-1)*(skip+1)+1)<=len(tweet)])
                    temp[skip]=list(set(all_skip_ngrams))
                all_skip_ngram[n]=temp
        print(len(all_skip_ngram))
        ...
        for tweet in preprocessed_train_tweets:
            ...
            for n,ngrams in all_skip_ngram.items():
                if n==1:
                    ...
                    for skip,ngram_array in ngrams.items():
                        tweet_skip=[' '.join(tweet[index:index+(n-1)*(skip+1)+1:(skip+1)]) for index in range(len(tweet)) if (index+(n-1)*(skip+1)+1)<=len(tweet)]
                        for ngram in ngram_array:
