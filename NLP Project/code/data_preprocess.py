...
x['lemmatized_Tweet']=get_stemmed_text(x['preclean_Tweet'])
x['stemmed_Tweet']=get_lemmatized_text(x['preclean_Tweet'])

# Preprocessing
'''
for m,i in enumerate(x.index[29173:]):
    print(m+29173)
    words=tweets[m].split(" ")
    if len(set(words).intersection(set(slang.keys())))<=0:
        x.loc[i,'noslang_Tweet']=tweets[m]
        continue
    else:
        for j,w in enumerate(words):
            if w in slang.keys():
                words[j]=slang.get(w)
        x.loc[i,'noslang_Tweet']=' '.join(words)
'''
x['noslang_Tweet']=[" ".join([slang.get(i,i) for i in t.split(" ")])  for t in tweets]
x['preclean_noslang_Tweet'] = preclean(x['noslang_Tweet'])
x['noslang_stemmed_Tweet'] = get_stemmed_text(x['preclean_noslang_Tweet'])
x['noslang_lemmatized_Tweet'] = get_lemmatized_text(x['preclean_noslang_Tweet'])
