...
from RankMetrics import MAPScorer

class CoordinateAscent(sklearn.base.BaseEstimator):
    """Coordinate Ascent"""

    def __init__(self, n_restarts=5, max_iter=25, tol=0.0001, scorer=None):
    ...
    def fit(self, X, y, qid, X_valid=None, y_valid=None, qid_valid=None):
        ...
        for restart_no in range(1, self.n_restarts + 1):
            coef = np.ones(X.shape[1], dtype=np.float64) / X.shape[1] #initial the same weights for every column
            #score = self.scorer(y, np.dot(X, coef), qid_valid).mean()
            score = MAPScorer(y, np.dot(X, coef), qid_valid)
            ...
            while n_fails < X.shape[1] - 1:
                for iter_no, fid in enumerate(np.random.permutation(X.shape[1]), 1):
                    ...
                    for j in range(self.max_iter):
                        #new_score = self.scorer(y, pred + change * pred_delta, qid_valid).mean()
                        new_score = MAPScorer(y, pred + change * pred_delta, qid_valid)
